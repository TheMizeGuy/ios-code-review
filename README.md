...
A concrete run of the default flow, from install to applied fix:
...
Team mode follows the same install → invoke → output → apply → verify shape; the difference is Step 3 (map → partition → show the plan → dispatch a wave of reviewers → runtime pass → seam review → consolidate) and a richer Step 4 output (partition table, per-agent attribution, seam findings, pattern findings).

## Troubleshooting

| Symptom | Cause | Fix |
|---|---|---|
| `claude plugin list` doesn't show `ios-senior-review@ios-code-review` | Restart step skipped, or the marketplace add failed silently | Re-run `claude plugin marketplace add https://github.com/TheMizeGuy/ios-code-review.git`, then `claude plugin install ios-senior-review@ios-code-review`, then fully restart Claude Code |
| Skill doesn't trigger from plain English | Phrasing didn't match a trigger and no explicit invocation was given | Use the explicit command form (`/ios-code-review:review-ios ...`) or rephrase closer to "review my iOS app" / "audit for App Store readiness" |
| "ios team review" ran standard mode instead of team mode | Team mode requires the exact phrase "ios team review" (case-insensitive) or `--team`; generic phrases like "team review" or "full audit" default to standard by design | Use the exact phrase or pass `--team` explicitly |
...
- **A local iOS knowledge base** — any directory of reference docs (Obsidian vault, markdown repo, WWDC notes). Tell the agent where it is in the dispatch prompt and it will read + cite relevant files alongside the canonical Apple sources.

Neither is required — the plugin's own `references/dimensions/` files plus the reviewer's built-in Apple-source citations cover every dimension on their own.
